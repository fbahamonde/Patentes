...
MAX_CHANGE_IN_HEIGHT = 0.17

MAX_ANGLE_BETWEEN_CHARS = 20.0

        # other constants
MIN_NUMBER_OF_MATCHING_CHARS = 4
#se agrega numero maximo de caracteres en linea 276
...
def getPNGfiles(FOLDER_PATH):
    ...
    for root, dirs, files in os.walk(FOLDER_PATH):
        ...
        for fichero in files:
            ...
            if extension == ".png":
                MPEGfiles.append(os.path.join(FOLDER_PATH,fichero))
    return MPEGfiles

def loadKNNDataAndTrainKNN():
    files = getPNGfiles('./CHARS')
    labels = np.asarray([os.path.basename(x)[0] for x in files ])
    images = [ cv2.imread(x,0) for x in files]
    images2 = np.asarray([ x.reshape(1,-1) for x in images])
    df = pd.DataFrame(images2[:,0])
    df['labels'] = labels
    y = df['labels']
...
##possiblePlate es una patente dentro de la lista de patentes donde possiblePlate.image es la imagen que se preprocesa
...
###ver si es bueno agregar ecualizacion solo a la patente
        #if Main.showSteps == True: # show steps ###################################################
         #   cv2.imshow("5a", possiblePlate.imgPlate)
          #  cv2.imshow("5b", possiblePlate.imgGrayscale)
           # cv2.imshow("5c", possiblePlate.imgThresh)
##possiblePlate.imgThresh es en blanco y negro
        # end if # show steps #####################################################################
        ...
        # end if # show steps #####################################################################
##aqui voy lo ultimo 15:49 06/06 (contronos de patente)
...
#aqui se hace el reconocimiento con knn
...
## lo que se le pasa al KNN
        ...
        npaResults =  kNearest.predict(npaROIResized)  # finally we can call findNearest !!!
    ...
    # # show steps #######################################################
    #cv2.imshow("Characters Detected", imgThreshColor)
    # end if # show steps #########################################################################
